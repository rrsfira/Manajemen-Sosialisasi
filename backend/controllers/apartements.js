...
router.post(
  ...
  (req, res) => {
    ...
    db.query("INSERT INTO apartments SET ?", payload, (err, result) => {
      if (err) {
      ...
      res
        .status(201)
        .json({
          message: "apartments ditambahkan",
          id: result.insertId,
        });
    });
  }
...
router.get("/:id", (req, res) => {
  ...
  db.query(sql, [id], (err, results) => {
    ...
    // Format URL SK
    if (result.SK) {
      result.SK_url = `${baseUrl}/uploads/apartments/sk/${encodeURIComponent(
        result.SK
      )}`;
    }
    ...
    try {
      ...
      if (Array.isArray(parsedPhoto)) {
        result.photo = parsedPhoto.map(
          (filename) =>
            `${baseUrl}/uploads/apartments/foto/${encodeURIComponent(filename)}`
        );
      } else {
        result.photo = [
          `${baseUrl}/uploads/apartments/foto/${encodeURIComponent(
            result.photo
          )}`,
        ];
      }
...
router.put(
  ...
  (req, res) => {
    ...
    // Ambil data lama
    const getOldQuery = "SELECT * FROM apartments WHERE id = ?";
...
// ✅ DELETE Education Unit// DELETE /apartments/:id
router.delete("/:id", verifyToken, (req, res) => {
  const { id } = req.params;
  const userId = req.user?.id;

  const selectSql = "SELECT photo, SK FROM apartments WHERE id = ?";
  db.query(selectSql, [id], (err, result) => {
    ...
    // Hapus SK
    if (SK) {
      const skPath = path.join(__dirname, "../uploads/apartments/sk", SK);
    ...
    photos.forEach((file) => {
      const filePath = path.join(__dirname, "../uploads/apartments/foto", file);
