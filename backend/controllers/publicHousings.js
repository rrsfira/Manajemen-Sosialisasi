...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    let folder = "uploads/public_housings";
...
`;
  ...
  db.query(query, (err, results) => {
    if (err) {
      console.error("Error fetching public_housings data:", err);
      res.status(500).send("Error fetching public_housings data");
    } else {
      res.json(results);
    }
  });
...
router.post(
  ...
  (req, res) => {
    ...
    db.query("INSERT INTO public_housings SET ?", payload, (err, result) => {
      if (err) {
      ...
      res
        .status(201)
        .json({
          message: "public_housings ditambahkan",
          id: result.insertId,
        });
    });
  }
...
router.get("/:id", (req, res) => {
  ...
  db.query(sql, [id], (err, results) => {
    ...
    // Format URL SK
    if (result.SK) {
      result.SK_url = `${baseUrl}/uploads/public_housings/sk/${encodeURIComponent(
        result.SK
      )}`;
    }
    ...
    try {
      ...
      if (Array.isArray(parsedPhoto)) {
        result.photo = parsedPhoto.map(
          (filename) =>
            `${baseUrl}/uploads/public_housings/foto/${encodeURIComponent(filename)}`
        );
      } else {
        result.photo = [
          `${baseUrl}/uploads/public_housings/foto/${encodeURIComponent(
            result.photo
          )}`,
        ];
      }
...
router.put(
  ...
  (req, res) => {
    ...
    // Ambil data lama
    const getOldQuery = "SELECT * FROM public_housings WHERE id = ?";
...
// ✅ DELETE Education Unit// DELETE /public_housings/:id
router.delete("/:id", verifyToken, (req, res) => {
  const { id } = req.params;
  const userId = req.user?.id;

  const selectSql = "SELECT photo, SK FROM public_housings WHERE id = ?";
  db.query(selectSql, [id], (err, result) => {
    ...
    // Hapus SK
    if (SK) {
      const skPath = path.join(__dirname, "../uploads/public_housings/sk", SK);
    ...
    photos.forEach((file) => {
      const filePath = path.join(__dirname, "../uploads/public_housings/foto", file);
