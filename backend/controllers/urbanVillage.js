...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    let folder = "uploads/urban_villages";
...
`;
  ...
  db.query(query, (err, results) => {
    if (err) {
      console.error("Error fetching urban_villages data:", err);
      res.status(500).send("Error fetching urban_villages data");
    } else {
      res.json(results);
    }
  });
...
router.post(
  ...
  (req, res) => {
    ...
    db.query("INSERT INTO urban_villages SET ?", payload, (err, result) => {
      if (err) {
      ...
      res
        .status(201)
        .json({
          message: "urban_villages ditambahkan",
          id: result.insertId,
        });
    });
  }
...
router.get("/:id", (req, res) => {
  ...
  db.query(sql, [id], (err, results) => {
    ...
    // Format URL SK
    if (result.SK) {
      result.SK_url = `${baseUrl}/uploads/urban_villages/sk/${encodeURIComponent(
        result.SK
      )}`;
    }
    ...
    try {
      ...
      if (Array.isArray(parsedPhoto)) {
        result.photo = parsedPhoto.map(
          (filename) =>
            `${baseUrl}/uploads/urban_villages/foto/${encodeURIComponent(filename)}`
        );
      } else {
        result.photo = [
          `${baseUrl}/uploads/urban_villages/foto/${encodeURIComponent(
            result.photo
          )}`,
        ];
      }
...
router.put(
  ...
  (req, res) => {
    ...
    // Ambil data lama
    const getOldQuery = "SELECT * FROM urban_villages WHERE id = ?";
...
// ✅ DELETE Education Unit// DELETE /urban_villages/:id
router.delete("/:id", verifyToken, (req, res) => {
  const { id } = req.params;
  const userId = req.user?.id;

  const selectSql = "SELECT photo, SK FROM urban_villages WHERE id = ?";
  db.query(selectSql, [id], (err, result) => {
    ...
    // Hapus SK
    if (SK) {
      const skPath = path.join(__dirname, "../uploads/urban_villages/sk", SK);
    ...
    photos.forEach((file) => {
      const filePath = path.join(__dirname, "../uploads/urban_villages/foto", file);
