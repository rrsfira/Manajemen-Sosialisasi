...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    let folder = "uploads/health_facilities";
...
router.get("/", (req, res) => {
  const query = `
  SELECT 
    hf.id, hf.name, hf.address, hf.category, hf.type, hf.leader, hf.activity, 
    hf.gender_man,
    hf.gender_women,
    hf.age_19to44years,
    hf.age_over44years,
...
router.post(
  ...
  (req, res) => {
    ...
    db.query("INSERT INTO health_facilities SET ?", payload, (err, result) => {
      if (err) {
      ...
      res
        .status(201)
        .json({
          message: "Fasilitas kesehatan ditambahkan",
          id: result.insertId,
        });
    });
  }
...
router.get("/:id", (req, res) => {
  ...
  db.query(sql, [id], (err, results) => {
    ...
    // Format URL SK
    if (result.SK) {
      result.SK_url = `${baseUrl}/uploads/health_facilities/sk/${encodeURIComponent(
        result.SK
      )}`;
    }
    ...
    try {
      ...
      if (Array.isArray(parsedPhoto)) {
        result.photo = parsedPhoto.map(
          (filename) =>
            `${baseUrl}/uploads/health_facilities/foto/${encodeURIComponent(filename)}`
        );
      } else {
        result.photo = [
          `${baseUrl}/uploads/health_facilities/foto/${encodeURIComponent(
            result.photo
          )}`,
        ];
      }
...
router.put(
  ...
  (req, res) => {
    ...
    const removePhotoFile = (filename) => {
      ...
      const filePath = path.join(
        __dirname,
        "../uploads/health_facilities/foto",
        filename
      );
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    };
    ...
    // Ambil data lama
    const getOldQuery = "SELECT * FROM health_facilities WHERE id = ?";
    db.query(getOldQuery, [id], (err, oldResults) => {
      ...
      const values = [
        ...
        fields.category || oldData.category,
        fields.type || oldData.type,
...
// ✅ DELETE Education Unit// DELETE /health_facilities/:id
router.delete("/:id", verifyToken, (req, res) => {
  const { id } = req.params;
  const userId = req.user?.id;

  const selectSql = "SELECT photo, SK FROM health_facilities WHERE id = ?";
  db.query(selectSql, [id], (err, result) => {
    ...
    // Hapus SK
    if (SK) {
      const skPath = path.join(__dirname, "../uploads/health_facilities/sk", SK);
    ...
    photos.forEach((file) => {
      const filePath = path.join(__dirname, "../uploads/health_facilities/foto", file);
