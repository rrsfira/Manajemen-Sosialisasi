...
router.post(
  ...
  (req, res) => {
    ...
    db.query("INSERT INTO hotels SET ?", payload, (err, result) => {
      if (err) {
...
router.get("/:id", (req, res) => {
  ...
  db.query(sql, [id], (err, results) => {
    ...
    // Format URL SK
    if (result.SK) {
      result.SK_url = `${baseUrl}/uploads/hotels/sk/${encodeURIComponent(
        result.SK
      )}`;
    }
    ...
    try {
      ...
      if (Array.isArray(parsedPhoto)) {
        result.photo = parsedPhoto.map(
          (filename) =>
            `${baseUrl}/uploads/hotels/foto/${encodeURIComponent(filename)}`
        );
      } else {
        result.photo = [
          `${baseUrl}/uploads/hotels/foto/${encodeURIComponent(
            result.photo
          )}`,
        ];
      }
...
router.put(
  ...
  (req, res) => {
    ...
    // Ambil data lama
    const getOldQuery = "SELECT * FROM hotels WHERE id = ?";
...
// ✅ DELETE Education Unit// DELETE /hotels/:id
router.delete("/:id", verifyToken, (req, res) => {
  const { id } = req.params;
  const userId = req.user?.id;

  const selectSql = "SELECT photo, SK FROM hotels WHERE id = ?";
  db.query(selectSql, [id], (err, result) => {
    ...
    // Hapus SK
    if (SK) {
      const skPath = path.join(__dirname, "../uploads/hotels/sk", SK);
    ...
    photos.forEach((file) => {
      const filePath = path.join(__dirname, "../uploads/hotels/foto", file);
