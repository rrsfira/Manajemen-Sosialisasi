...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    let folder = "uploads/education_units";
...
router.get("/", (req, res) => {
  const query = `
  SELECT 
    hf.id, hf.name, hf.address, hf.group, hf.instance, hf.leader, hf.activity, 
    hf.gender_man,
    hf.gender_women,
    hf.age_under6years,
    hf.age_6to10years,
...
`;
  ...
  db.query(query, (err, results) => {
    if (err) {
      console.error("Error fetching education_units data:", err);
      res.status(500).send("Error fetching education_units data");
    } else {
      res.json(results);
    }
  });
...
router.post(
  ...
  (req, res) => {
    ...
    db.query("INSERT INTO education_units SET ?", payload, (err, result) => {
      ...
      res.status(201).json({
        message: "Satuan Pendidikan ditambahkan",
        id: result.insertId,
      });
    });
  }
...
router.get("/:id", (req, res) => {
  ...
  db.query(sql, [id], (err, results) => {
    ...
    // Format URL SK
    if (result.SK) {
      result.SK_url = `${baseUrl}/uploads/education_units/sk/${encodeURIComponent(
        result.SK
      )}`;
    }
    ...
    try {
      ...
      if (Array.isArray(parsedPhoto)) {
        result.photo = parsedPhoto.map(
          (filename) =>
            `${baseUrl}/uploads/education_units/foto/${encodeURIComponent(
              filename
            )}`
        );
      } else {
    ...
    db.query(getOldQuery, [id], (err, oldResults) => {
      ...
      const updateSql = `
        UPDATE education_units SET 
          ...
          time = ?, gender_man = ?, gender_women = ?, age_under6years = ?, age_6to10years = ?, age_11to18years = ?, age_over44years = ?, 
          photo = ?, video = ?, SK = ?, 
...
router.delete("/:id", verifyToken, (req, res) => {
  ...
  db.query(selectSql, [id], (err, result) => {
    ...
    // Hapus SK
    if (SK) {
      const skPath = path.join(__dirname, "../uploads/education_units/sk", SK);
